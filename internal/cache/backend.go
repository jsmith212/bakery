...
//
// The registrar is `any` because grpc-go is not a dependency until M4, and adding a
// module to go.mod to type a method that nothing implements yet is the wrong trade.
// M4 narrows it to grpc.ServiceRegistrar; the assertion lives in the bazel backend,
// which is the only implementer.
//
// Note for M4: the project comes from the REAPI instance_name, which is
// "{org}/{project}" -- so it CONTAINS SLASHES. ByteStream resource names must be
// parsed by scanning for the blobs / uploads / compressed-blobs marker, never split
// positionally on '/'.
type GRPCBackend interface {
	Backend

	RegisterGRPC(registrar any) error
}
