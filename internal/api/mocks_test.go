...
// The SITE-role writes are faked on exactly the same terms as the org-membership ones
// above, and for the same reason: the effective site_role is
// coalesce(greatest(oidc, local), 'user'), GENERATED by the database, and a fake that
// computed it in Go would be asserting that this file agrees with itself. They record
// the call and refuse -- so an authorization test can still prove that a refused
// caller never reached the write, which is the thing a fake CAN prove.
func (s *fakeStore) ListSiteAdmins(_ context.Context) ([]repository.ListSiteAdminsRow, error) {
	s.note("ListSiteAdmins")

	if s.desiredErr != nil {
		return nil, s.desiredErr
	}

	return s.siteAdmins, nil
}

func (s *fakeStore) GrantSiteAdminLocal(
	_ context.Context, _ repository.GrantSiteAdminLocalParams,
) (repository.User, error) {
	s.note("GrantSiteAdminLocal")

	return repository.User{}, errFakeHybrid
}

func (s *fakeStore) RevokeSiteAdminLocal(_ context.Context, _ pgtype.UUID) (repository.User, error) {
	s.note("RevokeSiteAdminLocal")
